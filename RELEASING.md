...
2. Run the Elixir unit suite, core-only consumer, Mongo/MinIO integrations,
   generated documentation, and local archive validation.
3. Run the Ruby suite and RuboCop, including Mongo and S3 integrations.
...
GitHub Actions needs only its default repository token. Mongo and MinIO run as
ephemeral CI services; no external service secrets are required.
